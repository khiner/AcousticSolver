...
[DG usage](src/Dg/README.md) documents the strong–weak WADG method, formats, and
commands; [README.md](README.md#curved-tetrahedral-dg) reports timings.
Full-mass and WADG FP64 references check geometry, mass, spatial loads, and
factorization before FP32 export. All cases use `c = 343 m/s` and `rho = 1.2 kg/m³`.

## Independent acoustic solution

For cylinder radius `R`, height `L = 1 m`, and the first positive zero
`alpha = 3.8317059702075125` of `J1`:
...
`DgReference check` verifies the Bessel root, rigid-wall derivatives, regular
axis limit, Helmholtz equation, and acoustic equations. Independent Duffy–Gauss
quadrature scores `(p-2, rho*c*v)` against the exact field, excluding stationary
pressure from the denominator. Fixtures use degree 6 / 70 elements at radius 1 m
and degree 4 / 560 elements at radius 0.5 m, each with 1,024 steps of `2^-18 s`.

## Regression gates

- Analytic terminal field error below `1e-3`
- State and receiver-fluctuation errors against frozen FP64 below `2e-5`, using the FP64 modified mass norm for state
- Random mass relative error below `2e-5`; initial/random RHS errors below `2e-5`, scaled by `max(norm(reference_rhs), c*norm(state))`
...
for degree 4. With original modified mass `H = C C^T`, the selected inverse `A`
must satisfy `max|eig(C^T A C)-1| < 1e-6`, positive Jacobians/masses, and
constant-moment error below `1e-10`. These fixture-specific reductions leave
spatial and scoring quadratures unchanged.
...
Gmsh 4.15.2 generates the cylinder. Upstream prepares geometry in FP32; WADG
assembles operators in FP64 before rounding to FP32. The cube's 9³ and cylinder's
12³ mass quadratures pass the spectral/moment gates, with relative inverse mass
...
For the cylinder's 0.5 s run:

| Metric | Upstream CUDA | Metal WADG |
| --- | ---: | ---: |
| Terminal / initial physical energy | `0.999516` | `0.998766` |
| Physical mean drift / initial acoustic RMS | `1.24e-4` (terminal) | `4.21e-7` (maximum sampled) |
| Terminal field error against dense FP64 WADG | `0.525%` | `1.68e-5` |
| Complete receiver error against dense FP64 WADG | `0.191%` | `4.38e-6` |

CUDA exceeds the `1e-5` conservation gate. Metal passes both `2e-5` precision gates;
CUDA uses a different spatial formulation. The GPU implementations differ by
`0.526%` in terminal physical fields and `0.191%` in receiver records.
Metal's diagnostic and benchmark runs produce identical terminal/receiver bytes.
Repeats within each backend are byte-identical, and backend sample clocks match
exactly.

The stock degree-8 cylinder exceeds the FP32 initial-RHS tolerance and remains
outside this validated scope. These checks cover the stated meshes and schedules;
they establish no general timestep bound, per-stage energy theorem, or
convergence on other meshes.
