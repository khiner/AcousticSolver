...
Solver::Solver(const std::string &bubble_file, double dt, double ts) : Integ(dt), Dt(dt), Ts(ts) {
...
double Solver::Step() {
    ...
    while (EventIndex < EventTimes.size() && time >= EventTimes[EventIndex]) {
        if (time < EventTimes[EventIndex + 1]) {
            ...
            int uncoupled_idx = 0;
            for (Oscillator *osc : UncoupledOsc) {
            ...
            while (OscIndex < Oscillators.size() && time >= Oscillators[OscIndex].StartTime && Oscillators[OscIndex].StartTime < time2) {
                ...
                Oscillator *const osc = &Oscillators[OscIndex];
...
void Solver::MakeOscillators(const std::map<int, Bubble> &bubbles) {
    Oscillators.clear();
    ...
    for (const auto &[id, bubble] : bubbles) {
        ...
        int cur_id = id;
        const Bubble *cur = &bubble;

        Oscillator osc;
        osc.StartTime = cur->StartTime;

        std::vector<double> solve_times;
        std::vector<double> s_radii, s_w0, s_x, s_y, s_z;
        std::vector<double> c_vals; // Precomputed damping coefficient
        std::vector<double> force_times, f_cutoffs, f_weights;
        ...
        while (true) {
            ...
            if (cur->StartType == EventType::Split) {
                if (bubbles.at(cur->PrevIds.at(0)).Radius >= cur->Radius) force = Oscillator::CzerskiJetForcing(cur->Radius);
                ...
                if (all_merge) {
                    double r1 = bubbles.at(cur->PrevIds.at(0)).Radius;
                    double r2 = bubbles.at(cur->PrevIds.at(1)).Radius;

                    if (r1 + r2 > cur->Radius) {
                        double v1 = 4. / 3. * std::numbers::pi * r1 * r1 * r1;
                        double v2 = 4. / 3. * std::numbers::pi * r2 * r2 * r2;
                        const double vn = 4. / 3. * std::numbers::pi * cur->Radius * cur->Radius * cur->Radius;

                        const double diff = v1 + v2 - vn;
                        if (diff <= std::max(v1, v2)) {
                            ...
                            force = Oscillator::MergeForcing(cur->Radius, r1, r2);
                        }
                    } else {
                        force = Oscillator::MergeForcing(cur->Radius, r1, r2);
                    }
                ...
                force = Oscillator::CzerskiJetForcing(cur->Radius);
            }
            ...
            if (cur->EndType == EventType::Merge) {
                const Bubble &next = bubbles.at(cur->NextIds.at(0));
                ...
                std::multimap<double, int, std::greater<>> children;
        ...
        for (size_t i = 0; i < solve_times.size(); ++i) c_vals.push_back(2. * Oscillator::CalcBeta(s_radii[i], s_w0[i]));

        osc.SolveTimes = solve_times;
        osc.SolveData.resize(6, solve_times.size());
        osc.SolveData.row(0) = Eigen::Map<Eigen::VectorXd>(s_radii.data(), s_radii.size());
        osc.SolveData.row(1) = Eigen::Map<Eigen::VectorXd>(s_w0.data(), s_w0.size());
        osc.SolveData.row(2) = Eigen::Map<Eigen::VectorXd>(s_x.data(), s_x.size());
        osc.SolveData.row(3) = Eigen::Map<Eigen::VectorXd>(s_y.data(), s_y.size());
        osc.SolveData.row(4) = Eigen::Map<Eigen::VectorXd>(s_z.data(), s_z.size());
        osc.SolveData.row(5) = Eigen::Map<Eigen::VectorXd>(c_vals.data(), c_vals.size());

        osc.ForceData.resize(3, force_times.size());
        osc.ForceData.row(0) = Eigen::Map<Eigen::VectorXd>(force_times.data(), force_times.size());
        osc.ForceData.row(1) = Eigen::Map<Eigen::VectorXd>(f_cutoffs.data(), f_cutoffs.size());
        osc.ForceData.row(2) = Eigen::Map<Eigen::VectorXd>(f_weights.data(), f_weights.size());
