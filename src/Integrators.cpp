...
/** */
template<typename T>
void Integrator<T>::updateData(const std::vector<Oscillator<T> *> &coupled_osc, const std::vector<Oscillator<T> *> &uncoupled_osc, double time1, double time2) {
    ...
    _N_coupled = coupled_osc.size();
    _N_total = total_osc.size();
    _t1 = time1;
    _t2 = time2;

    // Set Oscillators solve data (radii, freq., xyz position, etc.) at endpoint times
    _solveData1.resize(6, _N_total);
    _solveData2.resize(6, _N_total);
    ...
    _forceData1.resize(3, _N_coupled);
    _forceData2.resize(3, _N_coupled);
...
void Integrator<T>::_computeKCF(double time) {
    ...
    double const alpha = (time - _t1) / (_t2 - _t1);
...
template<typename T>
void Coupled_Direct<T>::ConstructMass(const Eigen::Array<T, 6, Eigen::Dynamic> &solveData1, const Eigen::Array<T, 6, Eigen::Dynamic> &solveData2, double t1, double t2, double time, int N_coupled, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> &M) {
    double const alpha = (time - t1) / (t2 - t1);
...
void Coupled_Direct<T>::refactor() {
    ...
    ParallelFor(2, 1, [&](size_t which) {
        if (which == 0) {
            _constructMass(_t1, M1);
            _factor1.compute(M1);
        } else {
            _constructMass(_t2, M2);
            _factor2.compute(M2);
        }
    });
...
const Eigen::ArrayX<T> &Coupled_Direct<T>::solve(const Eigen::ArrayX<T> &States, double time) {
    ...
    if (_useInverse) {
        if (_N_coupled > 0) {
            _sol1.resize(_N_coupled);
            _sol2.resize(_N_coupled);
