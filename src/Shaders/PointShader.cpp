...
// (kernels: ImpulseToBoundary, ImpulseMatmul in Kernels.metal).
//
...
void PointSource::Compute(GpuBuffer &force, int global_bid) {
    ...
    ctx.Dispatch("ImpulseToBoundary", blocks, threads, {&ImpulseToBoundary, &base.GpuB, &base.GpuBN, &GpuImpulseData}, &itb_params, sizeof(itb_params));
    ...
    ctx.Dispatch("ImpulseMatmul", matmul_blocks, threads, {&force, &ImpulseToBoundary, &GpuImpulseVels}, &mm_params, sizeof(mm_params));
